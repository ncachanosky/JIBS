...
use "$path/data/clean-window-cases.dta", clear

// Merge firm observation counts:
merge m:1 iso year using "$path/data/compustat-country-clean.dta", ///
    keep(match) nogen

save "$path/data/master-stacked-country.dta", replace
...
save "$path/data/master-twfe-country.dta", replace
