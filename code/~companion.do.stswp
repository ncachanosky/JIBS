...
master-stacked-firm.dta that are useful for the paper's data section but are
not themselves part of the estimation. Read-only with respect to the
estimation samples used in estimates-sample-splits-polity--trend-ebalance-
restricted.do / -unrestricted.do -- this file never estimates anything, it
only describes the data those two files consume.
...
Run this after any rebuild of master-stacked-firm.dta, and again whenever the
episode/region composition changes (new case added, region dropped, etc.), so
the paper's data-description tables stay in sync with what's actually in the
estimation sample.
==============================================================================*/
//------------------------------------------------------------------------------
// Setup
...
//------------------------------------------------------------------------------
// Posts one row to the open postfile `pf': label, N treated firms, N control
// firms, N clusters (iso), and the dominant-episode share (% of treated firms
// coming from the single largest country) -- the diagnostic that motivated
// dropping Africa/Americas/Oceania and running the leave-one-episode-out
// checks in the estimation files, generalized here to every split at once.
capture program drop sample_stats
program define sample_stats
	args label treatvar ifcond
	quietly {
	...
	post `pf' ("`label'") (`nt') (`nc') (`ncl') ("`dom_country'") (`dom_share')
end
...
sample_stats "Baseline (full worldwide sample)" treat "1"
sample_stats "Left populists"  lpop "treat == 0 | lpop == 1"
sample_stats "Right populists" rpop "treat == 0 | rpop == 1"

quietly levelsof continent, local(continents)
foreach c of local continents {
	sample_stats "Continent: `c'" treat `"continent == "`c'""'
}

quietly levelsof _wb_region, local(wbregions)
foreach r of local wbregions {
	if "`r'" != "" {
		sample_stats "WB income group: `r'" treat `"_wb_region == "`r'""'
	}
}
...
// full-worldwide-sample ebalance call from each estimation file (not every
// subsample-specific call -- there are ~13 of those across both files, which
// would make this table unreadable) under both its slope target
// (estimates-sample-splits-polity--trend-ebalance-unrestricted.do) and its
// four-level-lag target (...--trend-ebalance-restricted.do), so both
// robustness-pair members are represented.
di as result "===================================================================="
...
quietly {
	...
	// --- Slope-target ebalance (matches ...-unrestricted.do Baseline) ---
	...
	// --- Level-target ebalance (matches ...-restricted.do Baseline) ---
